...
from sail_safe_functions.participant.preprocessing.select_sub_data_frame_precompute import SelectSubDataFramePrecompute
...
class DataFrameFederated:
    ...
    def select_sub_data_frame(
        ...
        for series_name in list_series_name:
            if series_name not in self.data_model_data_frame.dict_data_model_series:
                raise Exception(f"No such series: {series_name}")
        list_reference = self.map_function(SelectSubDataFramePrecompute, list_series_name)
        return DataFrameFederated(
            list_reference, self.data_model_data_frame.get_data_model_list_series(list_series_name)
        )

    def get_reference_data_frame(
        self,
        dataset_id: str,
    ) -> ReferenceDataFrame:
    ...
    @property
    def data_model_data_frame(
        self,
    ) -> DataModelDataFrame:
        return self.__data_model_data_frame

    @property
    def dict_reference_data_frame(
        self,
    ):
