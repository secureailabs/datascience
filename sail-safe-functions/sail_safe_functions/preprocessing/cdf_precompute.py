...
class CumulativeDistributionFunctionPrecompute:
    ...
    Warning this only works on series with unique values
    """
    ...
    def run(sample_0: pandas.Series, domain_min: float, domain_max: float) -> Tuple[List[float], List[float], int]:
        ...
        if size_sample_0 == 1:
            ...
            array_domain = numpy.append(array_domain, domain_max)
            array_value = numpy.arange(1, size_sample_0 + 1) / float(size_sample_0)
            array_value = numpy.insert(array_value, 0, 0)
            array_value = numpy.append(array_value, 1)

            array_domain_safe = numpy.linspace(domain_min, domain_max, count_value)
            # TODO this would work a lot better if we sampled from the other direction since diversity is guaranteed from there!!!!
            # So sameple from the value to the domain instead of from the domain to the values
