    ...
    def run(
        ...
        for series_name in data_frame_source.list_series_name:
            if series_name in list_series_name_impute:
                if (
                    is_string_dtype(data_frame_source[series_name])
                ...
                series_list = imputer.fit_transform(
                    data_frame_source.select_series([series_name])
                )[:, 0].tolist()
                series = Series(
                ...
                series.index = data_frame_source[
                    series_name
                ].index  # reattach index that sklearn removed
