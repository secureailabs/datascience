...
def tabular_to_float64(schema_data_frame: SchemaDataFrame, data_frame_source: DataFrameFederated) -> DataFrameFederated:
    return TabularToFloat64.run(schema_data_frame, data_frame_source)


class TabularToFloat64:
    def run(schema_data_frame: SchemaDataFrame, data_frame_source: DataFrameFederated) -> DataFrameFederated:
        check_instance(schema_data_frame, SchemaDataFrame)
        ...
        for dataset_id in data_frame_source.dict_dataframe:
            data_frame_target.dict_dataframe[dataset_id] = TabularToFloat64Precompute.run(
                schema_data_frame, data_frame_source.dict_dataframe[dataset_id]
            )
