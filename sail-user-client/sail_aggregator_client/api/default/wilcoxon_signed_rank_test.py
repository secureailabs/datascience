...
def _get_kwargs(
    *,
    ...
    url = "{}/statistics/wilcoxon_signed_rank_test".format(client.base_url)
...
) -> Optional[Union[HTTPValidationError, WilcoxonSignedRankTestResponseWilcoxonSignedRankTest]]:

    if response.status_code < 200 or response.status_code >= 300:
        raise Exception(f"Failure status code: {response.status_code}. Details: {response.text}")

    if response.status_code == HTTPStatus.OK:
        response_200 = WilcoxonSignedRankTestResponseWilcoxonSignedRankTest.from_dict(response.json())
...
def _build_response(
    *, client: Client, response: httpx.Response
) -> Response[Union[HTTPValidationError, WilcoxonSignedRankTestResponseWilcoxonSignedRankTest]]:
    return Response(
