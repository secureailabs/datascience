...
def _get_kwargs(
    *,
    ...
    url = "{}/data_ingestion/read_longitudinal_fhirv1".format(client.base_url)
...
) -> Optional[ReadLongitudinalFhirv1ResponseReadLongitudinalFhirv1]:

    if response.status_code < 200 or response.status_code >= 300:
        raise Exception(f"Failure status code: {response.status_code}. Details: {response.text}")

    if response.status_code == HTTPStatus.OK:
        response_200 = ReadLongitudinalFhirv1ResponseReadLongitudinalFhirv1.from_dict(response.json())
