...
def _get_kwargs(
    *,
    ...
    url = "{}/statistics_paired_t_test".format(client.base_url)
...
) -> Optional[Union[HTTPValidationError, StatisticsPairedTTestResponseStatisticsPairedTTest]]:

    if response.status_code < 200 or response.status_code >= 300:
        raise Exception(f"Failure status code: {response.status_code}. Details: {response.text}")

    if response.status_code == HTTPStatus.OK:
        response_200 = StatisticsPairedTTestResponseStatisticsPairedTTest.from_dict(response.json())
...
def _build_response(
    *, client: Client, response: httpx.Response
) -> Response[Union[HTTPValidationError, StatisticsPairedTTestResponseStatisticsPairedTTest]]:
    return Response(
